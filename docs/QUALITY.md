...
The accessibility/UX suite additionally exercises visible keyboard focus, reduced-motion behavior, semantic storefront progress and selections, accessible submission busy/error feedback, responsive overflow checks, authenticated admin keyboard traversal, modal/drawer Escape handling with focus trapping/restoration, custom combobox keyboard navigation, and keyboard-reachable image upload controls. Deterministic desktop/mobile screenshots are emitted as review evidence for representative storefront and authenticated admin sections. These checks are risk-focused regression evidence; they are not a claim of complete WCAG conformance or a substitute for manual visual review. Automated axe coverage is not currently part of the gate.
