...
The E2E workflow also installs an exact `axe-core@4.12.1` browser runtime only for the browser-test job. Representative loaded/error storefront plus authenticated admin/edit-dialog states are scanned on desktop/mobile, and serious or critical axe violations fail that surface. Axe is intentionally supplemental: behavior-specific keyboard/focus/dialog/overflow tests remain independent requirements, and these automated checks are not a claim of complete WCAG conformance or a substitute for manual visual review.
